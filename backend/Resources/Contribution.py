...
class ChamaContributionsResource(Resource):
    ...
    def post(self, chama_id):
        ...
        if contribution_date_raw:
            try:
                contribution_date = datetime.fromisoformat(contribution_date_raw)
            except ValueError:
...
class MyContributionHistoryResource(Resource):
    ...
    Logged-in member sees only their own contribution history for that chama.
    """
    @jwt_required()
    def get(self, chama_id):
        current_user = get_current_user()
...
class MemberContributionHistoryResource(Resource):
    ...
    Any active member can view a specific member's contribution history
    inside the same chama.
    """
    @jwt_required()
...
class ContributionUpdateResource(Resource):
    """
    PATCH /chamas/<int:chama_id>/contributions/<int:contribution_id>

    Only admin or treasurer can update a contribution record.
    """
    @jwt_required()
    def patch(self, chama_id, contribution_id):
        ...
        if "contribution_date" in data:
            ...
            if raw_date:
                try:
                    contribution.contribution_date = datetime.fromisoformat(raw_date)
                except ValueError:
                    return {"message": "Invalid contribution_date. Use ISO format."}, 400
...
class ContributionDeleteResource(Resource):
    """
    DELETE /chamas/<int:chama_id>/contributions/<int:contribution_id>

    Only admin or treasurer can delete a contribution record.
    """
    @jwt_required()
...
class ContributionSummaryResource(Resource):
    ...
    Any active member can view summary statistics.
    """
    @jwt_required()
    def get(self, chama_id):
        current_user = get_current_user()
        ...
        member_summaries.sort(
            key=lambda x: (x["total_contributed"], x["contribution_count"]),
            reverse=True
        )
